...
try {
	...
	if(isset($_POST['navbar_submit'])) {
		try {
			...
			if(!isset($_POST['navbar_id'])) {
				throw new RuntimeException('Please fill all the form fields.');
			}
			if(!isset($_POST['navbar_type'])) {
				throw new RuntimeException('Please fill all the form fields.');
			}
			if(!isset($_POST['navbar_phrase'])) {
				throw new RuntimeException('Please fill all the form fields.');
			}
			if(!in_array($_POST['navbar_type'], array('internal','external'))) {
				throw new RuntimeException('Link type is not valid.');
			}
			if(!in_array($_POST['navbar_visibility'], array('user','guest','always'))) {
