...
function saveChanges(): void {
	...
	$csConfig = json_decode($csConfig, true, 512, JSON_THROW_ON_ERROR);
	if(!is_array($csConfig)) {
		throw new RuntimeException('Error loading config file.');
	}
	
	// active
	if(!Validator::UnsignedNumber($_POST['setting_1'])) {
		throw new RuntimeException('Submitted setting is not valid (active)');
	}
	...
	if(!Validator::UnsignedNumber($_POST['setting_2'])) {
		throw new RuntimeException('Submitted setting is not valid (hide_idle)');
	}
	if(!in_array($_POST['setting_2'], array('0','1',0,1))) {
		throw new RuntimeException('Submitted setting is not valid (hide_idle)');
	}
	...
	if(!Validator::UnsignedNumber($_POST['setting_3'])) {
		throw new RuntimeException('Submitted setting is not valid (live_data)');
	}
	if(!in_array($_POST['setting_3'], array('0','1',0,1))) {
		throw new RuntimeException('Submitted setting is not valid (live_data)');
	}
	...
	if(!Validator::UnsignedNumber($_POST['setting_4'])) {
		throw new RuntimeException(
				'Submitted setting is not valid (show_castle_owner)'
		);
	}
	if(!in_array($_POST['setting_4'], array('0','1',0,1))) {
		throw new RuntimeException(
				'Submitted setting is not valid (show_castle_owner)'
		);
	}
	...
	if(!Validator::UnsignedNumber($_POST['setting_5'])) {
		throw new RuntimeException(
				'Submitted setting is not valid (show_castle_owner_alliance)'
		);
	}
	if(!in_array($_POST['setting_5'], array('0','1',0,1))) {
		throw new RuntimeException(
				'Submitted setting is not valid (show_castle_owner_alliance)'
		);
	}
	...
	if(!Validator::UnsignedNumber($_POST['setting_6'])) {
		throw new RuntimeException(
				'Submitted setting is not valid (show_battle_countdown)'
		);
	}
	if(!in_array($_POST['setting_6'], array('0','1',0,1))) {
		throw new RuntimeException(
				'Submitted setting is not valid (show_battle_countdown)'
		);
	}
	...
	if(!Validator::UnsignedNumber($_POST['setting_7'])) {
		throw new RuntimeException(
				'Submitted setting is not valid (show_castle_information)'
		);
	}
	if(!in_array($_POST['setting_7'], array('0','1',0,1))) {
		throw new RuntimeException(
				'Submitted setting is not valid (show_castle_information)'
		);
	}
	...
	if(!Validator::UnsignedNumber($_POST['setting_8'])) {
		throw new RuntimeException(
				'Submitted setting is not valid (show_current_stage)'
		);
	}
	if(!in_array($_POST['setting_8'], array('0','1',0,1))) {
		throw new RuntimeException(
				'Submitted setting is not valid (show_current_stage)'
		);
	}
	...
	// show_next_stage
	if(!Validator::UnsignedNumber($_POST['setting_9'])) {
		throw new RuntimeException('Submitted setting is not valid (show_next_stage)');
	}
	if(!in_array($_POST['setting_9'], array('0','1',0,1))) {
		throw new RuntimeException('Submitted setting is not valid (show_next_stage)');
	}
	$csConfig['show_next_stage'] = $_POST['setting_9'];
	
	// show_battle_duration
	if(!Validator::UnsignedNumber($_POST['setting_10'])) {
		throw new RuntimeException(
				'Submitted setting is not valid (show_battle_duration)'
		);
	}
	if(!in_array($_POST['setting_10'], array('0','1',0,1))) {
		throw new RuntimeException(
				'Submitted setting is not valid (show_battle_duration)'
		);
	}
	...
	if(!Validator::UnsignedNumber($_POST['setting_11'])) {
		throw new RuntimeException(
				'Submitted setting is not valid (show_registered_guilds)'
		);
	}
	if(!in_array($_POST['setting_11'], array('0','1',0,1))) {
		throw new RuntimeException(
				'Submitted setting is not valid (show_registered_guilds)'
		);
	}
	...
	// show_schedule
	if(!Validator::UnsignedNumber($_POST['setting_12'])) {
		throw new RuntimeException('Submitted setting is not valid (show_schedule)');
	}
	if(!in_array($_POST['setting_12'], array('0','1',0,1))) {
		throw new RuntimeException('Submitted setting is not valid (show_schedule)');
	}
	$csConfig['show_schedule'] = $_POST['setting_12'];
	
	// schedule_date_format
	if(!isset($_POST['setting_13'])) {
		throw new RuntimeException(
				'Submitted setting is not valid (schedule_date_format)'
		);
	}
	$csConfig['schedule_date_format'] = $_POST['setting_13'];
	
	// show_widget
	if(!Validator::UnsignedNumber($_POST['setting_14'])) {
		throw new RuntimeException('Submitted setting is not valid (show_widget)');
	}
	if(!in_array($_POST['setting_14'], array('0','1',0,1))) {
		throw new RuntimeException('Submitted setting is not valid (show_widget)');
	}
	...
	// start_day
	if(count($_POST['setting_stage_startday']) != count($csConfig['stages'])) {
		throw new RuntimeException(
				'Schedule stages settings array size is not valid.'
		);
	}
	...
	// start_time
	if(count($_POST['setting_stage_starttime']) != count($csConfig['stages'])) {
		throw new RuntimeException(
				'Schedule stages settings array size is not valid.'
		);
	}
	...
	// end_day
	if(count($_POST['setting_stage_endday']) != count($csConfig['stages'])) {
		throw new RuntimeException(
				'Schedule stages settings array size is not valid.'
		);
	}
	...
	// end_time
	if(count($_POST['setting_stage_endtime']) != count($csConfig['stages'])) {
		throw new RuntimeException(
				'Schedule stages settings array size is not valid.'
		);
	}
	...
	$fp = fopen($cfgFile, 'wb');
	if(!fwrite($fp,
			json_encode($csConfig, JSON_THROW_ON_ERROR | JSON_PRETTY_PRINT)
	)) {
