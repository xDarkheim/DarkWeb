...
try {
	...
	if(isset($_REQUEST['cache']) && $_REQUEST['cache'] == 1) {
		...
		if(!$cacheNews) {
			throw new RuntimeException('The news could not be cached.');
		}
	...
	echo '<form role="form" method="post">';
		...
		echo '<div class="form-group">';
			...
			echo '<select class="form-control" name="news_language" id="input_1">';
				...
				foreach($languagesList as $language) {
					if($language == config('language_default', true)) {
						continue;
					}
		...
		echo '<div class="form-group">';
			...
			echo '<input type="text" class="form-control" id="input_2" name="news_title" value="'.($_POST['news_title']
							?? $newsData['news_title']).'" />';
		echo '</div>';
		echo '<div class="form-group">';
			echo '<label for="news_content"></label>';
			echo '<textarea name="news_content" id="news_content">'.($_POST['news_content']
							?? $newsData['news_content']).'</textarea>';
		echo '</div>';
