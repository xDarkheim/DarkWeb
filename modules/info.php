...
$infoServerName    = htmlspecialchars(config('server_name',      true) ?? 'MU Online');
$infoSeason        = htmlspecialchars(config('server_info_season',   true) ?? '');
$infoExpType       = htmlspecialchars(config('server_info_exp_type', true) ?? '');
$infoMaxLevel      = htmlspecialchars(config('server_info_max_level',true) ?? '400');
$infoMaxReset      = htmlspecialchars(config('server_info_max_reset',true) ?? '—');
$infoExp           = htmlspecialchars(config('server_info_exp',      true) ?? '—');
$infoMasterExp     = htmlspecialchars(config('server_info_masterexp',true) ?? '—');
$infoDrop          = htmlspecialchars(config('server_info_drop',     true) ?? '—');
?>

<!-- HERO BANNER -->
<div class="info-hero">
	<div class="info-hero-text">
		<div class="info-hero-title">Welcome to <?php echo $infoServerName; ?></div>
		<div class="info-hero-sub"><?php echo $infoSeason; ?> &mdash; <?php echo $infoExpType; ?> server. A realm of dark fantasy, epic battles and legendary power awaits. Rise through the ranks, forge powerful alliances and conquer the forces of darkness.</div>
	</div>
</div>
...
<div class="info-rates-grid">
	<div class="info-rate-card">
		...
		<div class="info-rate-value"><?php echo $infoExp; ?></div>
	</div>
	<div class="info-rate-card">
		<div class="info-rate-icon">🧿</div>
		<div class="info-rate-label">Master Experience</div>
		<div class="info-rate-value"><?php echo $infoMasterExp; ?></div>
	</div>
	<div class="info-rate-card">
		<div class="info-rate-icon">📦</div>
		<div class="info-rate-label">Item Drop</div>
		<div class="info-rate-value"><?php echo $infoDrop; ?></div>
	</div>
	<div class="info-rate-card">
		<div class="info-rate-icon">🎯</div>
		<div class="info-rate-label">Max Level</div>
		<div class="info-rate-value"><?php echo $infoMaxLevel; ?></div>
	</div>
	<div class="info-rate-card">
		<div class="info-rate-icon">🔄</div>
		<div class="info-rate-label">Max Reset</div>
		<div class="info-rate-value"><?php echo $infoMaxReset; ?></div>
	</div>
	<div class="info-rate-card">
		<div class="info-rate-icon">🌐</div>
		<div class="info-rate-label">Season</div>
		<div class="info-rate-value" style="font-size:14px;"><?php echo $infoSeason; ?></div>
	</div>
</div>
