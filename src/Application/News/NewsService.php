...
class NewsService
{
    ...
    public function setLanguage($language): void
    {
        ...
        if (! in_array($language, $languagesList, true)) {
            return;
        }
    ...
    public function addNews($title, $content, $author = 'Administrator', $comments = 1): void
    {
        ...
        if (! $this->checkTitle($title)) {
            MessageRenderer::toast('error', Translator::phrase('error_42'));
            return;
        }
        if (! $this->checkContent($content)) {
            MessageRenderer::toast('error', Translator::phrase('error_43'));
            return;
        }
    ...
    {
        $this->db = Connection::Database('MuOnline');
        if (! Validator::hasValue($id) || ! Validator::hasValue($title) || ! Validator::hasValue($content) || ! Validator::hasValue($author) || ! Validator::hasValue($comments) || ! Validator::hasValue($date)) {
            return;
        }
        if (! $this->newsIdExists($id)) {
            return;
    ...
    public function cacheNews(): bool
    {
        ...
        foreach ($news_list as $news) {
            $handle = fopen(
                __PATH_NEWS_CACHE__ . 'news_' . $news['news_id'] . '.cache',
                'ab',
            );
            ...
            if ($this->_enableShortNews) {
                $handle2 = fopen(
                    __PATH_NEWS_CACHE__ . 'news_' . $news['news_id'] . '_s.cache',
                    'ab',
                );
    ...
    {
        $this->db = Connection::Database('MuOnline');
        if (! Validator::hasValue($id) || ! $this->newsIdExists($id)) {
            return;
        }
    ...
    public function addNewsTransation(): void
    {
        $this->db = Connection::Database('MuOnline');
        if (! Validator::hasValue($this->_id)) {
            throw new \Exception('The provided news id is not valid.');
        }
        if (! Validator::hasValue($this->_language)) {
            throw new \Exception('The provided news language is not valid.');
        }
        if (! Validator::hasValue($this->_title)) {
            throw new \Exception('The provided news title is not valid.');
        }
        if (! Validator::hasValue($this->_content)) {
            throw new \Exception('The provided news content is not valid.');
        }
        ...
        if (! $result) {
            throw new \Exception('Could not add the news translation.');
        }
    ...
    public function updateNewsTransation(): void
    {
        ...
        if (! $result) {
            throw new \Exception('Could not update the news translation.');
        }
    ...
    public function deleteNewsTranslation(): void
    {
        ...
        if (! $result) {
            throw new \Exception('Could not delete news translation.');
        }
    ...
    {
        $this->db = Connection::Database('MuOnline');
        if (! Validator::hasValue($this->_id) || ! Validator::hasValue($this->_language)) {
            return;
        }
