...
class Account extends Common
{
    ...
    private function createRegistrationVerification($username, $password, $email)
    {
        ...
        $key    = uniqid('', true);
        $result = $this->muonline->query("INSERT INTO " . Register_Account . " (registration_account,registration_password,registration_email,registration_date,registration_ip,registration_key) VALUES (?,?,?,?,?,?)", [$username, $password, $email, time(), $_SERVER['REMOTE_ADDR'], $key]);
        if (!$result) return;
        return $key;
    }
