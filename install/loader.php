...
try {
        ...
        if(!@include(__PATH_CONFIGS__ . 'cms.tables.php')) throw new Exception('Could not load DarkWeb table definitions.');
        if(!@include(__INSTALL_ROOT__ . 'definitions.php')) throw new Exception('Could not load DarkWeb Installer definitions.');

	$cmsConfigsPath = __PATH_CONFIGS__ . CMS_CONFIGURATION_FILE;
	if(!file_exists($cmsConfigsPath)) throw new Exception('DarkWeb configuration file missing.');
	if(!is_readable($cmsConfigsPath)) throw new Exception('DarkWeb configuration file is not readable.');
	if(!is_writable($cmsConfigsPath)) throw new Exception('DarkWeb configuration file is not writable.');
	...
	if($cmsConfigsFile) {
		...
		if(!is_array($cmsConfig)) throw new Exception('DarkWeb configuration file could not be decoded.');
		if($cmsConfig['cms_installed'] === true) throw new Exception('DarkWeb installation is complete. It is recommended to rename or delete the install directory.');
	}

	$cmsDefaultConfigsPath = __PATH_CONFIGS__ . CMS_DEFAULT_CONFIGURATION_FILE;
	if(!file_exists($cmsDefaultConfigsPath)) throw new Exception('DarkWeb default configuration file missing.');
	if(!is_readable($cmsDefaultConfigsPath)) throw new Exception('DarkWeb default configuration file is not readable.');
	$cmsDefaultConfigsFile = file_get_contents($cmsDefaultConfigsPath);
	if(!$cmsDefaultConfigsFile) throw new Exception('DarkWeb default configuration file could not be loaded.');
	...
	if(!is_array($cmsDefaultConfig)) throw new Exception('DarkWeb default configuration file could not be decoded.');

        if(!@include(__PATH_INCLUDES__ . 'functions.php')) throw new Exception('Could not load DarkWeb functions.');
        if(!@include(__PATH_CONFIGS__ . 'timezone.php')) throw new Exception('Could not load DarkWeb timezone.');
	...
	if(!is_array($writablePaths)) throw new Exception('Could not load DarkWeb writable paths list.');
