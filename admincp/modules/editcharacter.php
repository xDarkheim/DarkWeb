...
if(isset($_GET['name'])) {
	try {
			...
			try {
				...
				if(!Validator::UnsignedNumber($_POST['characteredit_class'])) {
					throw new RuntimeException(
						"All the entered values must be numeric."
					);
				}
				...
				if(!Validator::UnsignedNumber($_POST['characteredit_mlexp'])) {
					throw new RuntimeException(
						"All the entered values must be numeric."
					);
				}
				...
				if(!Validator::UnsignedNumber($_POST['characteredit_mlpoint'])) {
					throw new RuntimeException(
						"All the entered values must be numeric."
					);
				}
				...
				$query = "UPDATE "._TBL_CHR_." SET ";
					...
					if(check_value($updateData['resets'])) {
						$query .= _CLMN_CHR_RSTS_." = :resets,";
					}
					if(check_value($updateData['gresets'])) {
				...
				if(!$updateCharacter) {
					throw new RuntimeException(
						"Master level data could not be updated."
					);
				}
		...
		if(!$charData) {
			throw new RuntimeException(
				"Could not retrieve character information (invalid character)."
			);
		}
				...
				if(defined('_TBL_MASTERLVL_')) {
					...
					echo '<div class="panel-body">';
						if(is_array($mLinfo)) {
							...
							inline_message('info', 'Could not retrieve Master Level information.');
						}
					echo '</div>';
					echo '</div>';
				}
